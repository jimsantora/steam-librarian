...
from ..user_context import format_user_context_error, resolve_user_context
...
async def read_library_overview() -> ResourceContents:
    ...
    if "error" in user_context:
        error_text = format_user_context_error(user_context)
        return TextResourceContents(
            uri="library://overview",
            mimeType="application/json",
    ...
    stats = await cache.get_or_compute(
        cache_key,
        lambda: calculate_library_overview(user),
        ttl=1800  # 30 minutes
    )
...
async def read_game_details(app_id: str) -> ResourceContents:
    ...
    # Get game details
    details = await cache.get_or_compute(
        cache_key,
        lambda: get_game_details_with_context(app_id, user),
        ttl=3600  # 1 hour
    )
...
async def read_recent_activity() -> ResourceContents:
    ...
    # Get recent activity
    activity = await cache.get_or_compute(
        cache_key,
        lambda: get_recent_activity(user),
...
async def read_genre_games(genre_name: str) -> ResourceContents:
    ...
    if "error" in user_context:
        return TextResourceContents(
            uri=f"library://genres/{genre_name}",
            mimeType="application/json",
            text=json.dumps(user_context, indent=2)
        )
    ...
    # Get games by genre
    games = await cache.get_or_compute(
        cache_key,
        lambda: get_games_by_genre(user, genre_name),
        ttl=1800  # 30 minutes
    )
