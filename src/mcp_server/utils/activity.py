...
def get_recent_activity(user: UserProfile) -> dict[str, Any]:
    ...
    with get_db() as session:
        ...
        ).order_by(UserGame.playtime_2weeks.desc()).all()
        ...
        for ug in recent_games:
            if ug.game:
                activity = {
                    ...
                    "total_playtime_hours": round(ug.playtime_forever / 60, 1),
        ...
        most_played_alltime = session.query(UserGame).options(
        ...
        most_played = [
            {
                ...
                "is_recent": ug.playtime_2weeks > 0
            }
            for ug in most_played_alltime if ug.game
        ]

        # Calculate activity summary
        summary = {
            "total_recent_hours": round(total_recent_playtime / 60, 1),
            "active_games_count": len(recent_games),
            "average_session_estimate": round(total_recent_playtime / (14 * len(recent_games)), 1) if recent_games else 0,
            "timestamp": datetime.now().isoformat()
        }

        return {
            "recent_games": recent_activity,
            "most_played_alltime": most_played,
            "summary": summary
        }
