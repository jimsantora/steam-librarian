...
async def get_game_details_with_context(app_id: str, user: UserProfile | None = None) -> dict[str, Any]:
    ...
    with get_db() as session:
        ...
        ).filter_by(app_id=int(app_id)).first()
        ...
        if game.reviews:
            details["reviews"] = {
                ...
                "positive": game.reviews.positive_reviews,
                "negative": game.reviews.negative_reviews,
        ...
        if user:
            ...
            if user_game:
                details["your_history"] = {
                    "owned": True,
                    "playtime_hours": round(user_game.playtime_forever / 60, 1),
                    "recent_playtime_hours": round(user_game.playtime_2weeks / 60, 1),
                    "last_played": "Recently" if user_game.playtime_2weeks > 0 else "Not recently"
                }
            else:
                details["your_history"] = {
                    "owned": False,
                    "playtime_hours": 0,
                    "recent_playtime_hours": 0,
                    "last_played": "Never"
                }
        ...
        details["multiplayer_info"] = {
            "is_multiplayer": is_multiplayer,
            "types": [cat.category_name for cat in game.categories if cat.category_name in multiplayer_categories]
        }
