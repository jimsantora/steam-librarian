...
def get_all_users() -> list[dict[str, Any]]:
    ...
    with get_db() as session:
        ...
        return [{"steam_id": user.steam_id, "persona_name": user.persona_name or "Unknown", "profile_url": user.profile_url or "", "steam_level": user.steam_level or 0, "last_updated": user.last_updated} for user in users]
...
def get_user_info(user_steam_id: Annotated[str | None, "Steam ID or persona name of user (leave empty for auto-selection)"] = None) -> dict[str, Any] | None:
    ...
    if not steam_id:
        return {"error": "No Steam ID configured", "steam_id": "", "persona_name": "Unknown", "profile_url": "", "steam_level": 0, "xp": 0, "account_age_years": 0, "account_created_date": "", "location": "Unknown", "avatar_urls": {"small": "", "medium": "", "large": ""}}

    with get_db() as session:
        user = session.query(UserProfile).filter_by(steam_id=steam_id).first()

        if not user:
            return {"error": "User profile not found. Run steam_library_fetcher.py first.", "steam_id": steam_id, "persona_name": "Unknown", "profile_url": "", "steam_level": 0, "xp": 0, "account_age_years": 0, "account_created_date": "", "location": "Unknown", "avatar_urls": {"small": "", "medium": "", "large": ""}}
...
def search_games(query: Annotated[str, "Search term to match against game name, genre, developer, or publisher"], user_steam_id: Annotated[str | None, "Steam ID or persona name of user (leave empty for auto-selection)"] = None) -> list[dict[str, Any]]:
    ...
    with get_db() as session:
        ...
        results = session.query(Game.app_id, Game.name, func.group_concat(Genre.genre_name.distinct()).label("genres"), GameReview.review_summary, UserGame.playtime_forever).join(UserGame, Game.app_id == UserGame.app_id).outerjoin(GameReview, Game.app_id == GameReview.app_id).outerjoin(Game.genres).outerjoin(Game.developers).outerjoin(Game.publishers).filter(and_(UserGame.steam_id == steam_id, or_(Game.name.ilike(query_lower), Genre.genre_name.ilike(query_lower), Developer.developer_name.ilike(query_lower), Publisher.publisher_name.ilike(query_lower)))).group_by(Game.app_id).order_by(desc(UserGame.playtime_forever)).all()
...
def filter_games(playtime_min: Annotated[float | None, "Minimum playtime in hours"] = None, playtime_max: Annotated[float | None, "Maximum playtime in hours"] = None, review_summary: Annotated[str | None, "Review summary to filter by (e.g., 'Very Positive', 'Overwhelmingly Positive')"] = None, maturity_rating: Annotated[str | None, "Maturity rating to filter by (e.g., 'Everyone', 'Teen (13+)')"] = None, user_steam_id: Annotated[str | None, "Steam ID of user (leave empty to be prompted)"] = None) -> list[dict[str, Any]]:
    ...
    with get_db() as session:
        query = session.query(Game.app_id, Game.name, func.group_concat(Genre.genre_name.distinct()).label("genres"), GameReview.review_summary, UserGame.playtime_forever).join(UserGame, Game.app_id == UserGame.app_id).outerjoin(GameReview, Game.app_id == GameReview.app_id).outerjoin(Game.genres).filter(UserGame.steam_id == steam_id)
...
def get_game_details(game_identifier: Annotated[str, "Game name or appid to get details for"], user_steam_id: Annotated[str | None, "Steam ID of user (leave empty to be prompted)"] = None) -> dict[str, Any] | None:
    ...
    with get_db() as session:
        ...
        # Get user game data
        user_game = session.query(UserGame).filter_by(steam_id=steam_id, app_id=game.app_id).first()
...
def get_library_stats(user_steam_id: Annotated[str | None, "Steam ID or persona name of user (leave empty for auto-selection)"] = None) -> dict[str, Any]:
    ...
    steam_id = user_result["steam_id"]
    if not steam_id:
        return {"total_games": 0, "total_hours_played": 0, "average_hours_per_game": 0, "top_genres": {}, "top_developers": {}, "review_distribution": {}}
    ...
    with get_db() as session:
        ...
        # Top genres
        genre_stats = session.query(Genre.genre_name, func.count(Genre.genre_name).label("count")).join(Game.genres).join(UserGame, Game.app_id == UserGame.app_id).filter(UserGame.steam_id == steam_id).group_by(Genre.genre_name).order_by(desc("count")).limit(10).all()

        top_genres = {genre.genre_name: genre.count for genre in genre_stats}

        # Top developers
        dev_stats = session.query(Developer.developer_name, func.count(Developer.developer_name).label("count")).join(Game.developers).join(UserGame, Game.app_id == UserGame.app_id).filter(UserGame.steam_id == steam_id).group_by(Developer.developer_name).order_by(desc("count")).limit(10).all()

        top_developers = {dev.developer_name: dev.count for dev in dev_stats}

        # Review distribution
        review_stats = session.query(GameReview.review_summary, func.count(GameReview.review_summary).label("count")).join(UserGame, GameReview.app_id == UserGame.app_id).filter(UserGame.steam_id == steam_id).group_by(GameReview.review_summary).all()
...
def get_recently_played(user_steam_id: Annotated[str | None, "Steam ID of user (leave empty to be prompted)"] = None) -> list[dict[str, Any]]:
    ...
    with get_db() as session:
        results = session.query(Game.app_id, Game.name, UserGame.playtime_2weeks, UserGame.playtime_forever).join(UserGame, Game.app_id == UserGame.app_id).filter(and_(UserGame.steam_id == steam_id, UserGame.playtime_2weeks > 0)).order_by(desc(UserGame.playtime_2weeks)).all()
...
def get_recommendations(user_steam_id: Annotated[str | None, "Steam ID of user (leave empty to be prompted)"] = None) -> list[dict[str, Any]]:
    ...
    with get_db() as session:
        ...
        played_games = session.query(UserGame).filter(and_(UserGame.steam_id == steam_id, UserGame.playtime_forever > 0)).count()

        if played_games == 0:
            # If no games played, recommend highest rated games
            top_rated = session.query(Game).join(GameReview, Game.app_id == GameReview.app_id).join(UserGame, Game.app_id == UserGame.app_id).filter(and_(UserGame.steam_id == steam_id, UserGame.playtime_forever == 0, GameReview.review_summary.in_(["Overwhelmingly Positive", "Very Positive"]))).limit(5).all()

            for game in top_rated:
                review = session.query(GameReview).filter_by(app_id=game.app_id).first()
                recommendations.append({"appid": game.app_id, "name": game.name, "reason": f"Highly rated game ({review.review_summary if review else 'Unknown'}) you haven't played yet"})
            return recommendations

        # Find favorite genres by total playtime
        genre_playtime = session.query(Genre.genre_name, func.sum(UserGame.playtime_forever).label("total_playtime")).join(Game.genres).join(UserGame, Game.app_id == UserGame.app_id).filter(and_(UserGame.steam_id == steam_id, UserGame.playtime_forever > 0)).group_by(Genre.genre_name).order_by(desc("total_playtime")).limit(3).all()

        # Find unplayed games in favorite genres
        for genre_data in genre_playtime:
            genre_games = session.query(Game).join(Game.genres).join(UserGame, Game.app_id == UserGame.app_id).outerjoin(GameReview, Game.app_id == GameReview.app_id).filter(and_(UserGame.steam_id == steam_id, UserGame.playtime_forever == 0, Genre.genre_name == genre_data.genre_name, or_(GameReview.review_summary.in_(["Overwhelmingly Positive", "Very Positive"]), GameReview.review_summary.is_(None)))).limit(2).all()

            hours = round(genre_data.total_playtime / 60, 1)
            for game in genre_games:
                recommendations.append({"appid": game.app_id, "name": game.name, "reason": f"Similar genre ({genre_data.genre_name}) to games you've played {hours} hours"})

        # Find games from favorite developers
        dev_playtime = session.query(Developer.developer_name, func.sum(UserGame.playtime_forever).label("total_playtime")).join(Game.developers).join(UserGame, Game.app_id == UserGame.app_id).filter(and_(UserGame.steam_id == steam_id, UserGame.playtime_forever > 0)).group_by(Developer.developer_name).order_by(desc("total_playtime")).limit(3).all()

        for dev_data in dev_playtime:
            dev_games = session.query(Game).join(Game.developers).join(UserGame, Game.app_id == UserGame.app_id).filter(and_(UserGame.steam_id == steam_id, UserGame.playtime_forever == 0, Developer.developer_name == dev_data.developer_name)).limit(1).all()

            hours = round(dev_data.total_playtime / 60, 1)
            for game in dev_games:
                recommendations.append({"appid": game.app_id, "name": game.name, "reason": f"From {dev_data.developer_name} who made games you've played {hours} hours"})
...
def get_friends_data(data_type: Annotated[str, "Type of data: 'list', 'common_games', 'who_owns_game', 'library_comparison'"], user_steam_id: Annotated[str | None, "Steam ID or persona name of user (leave empty for auto-selection)"] = None, friend_steam_id: Annotated[str | None, "Friend's Steam ID or persona name for specific queries"] = None, game_identifier: Annotated[str | None, "Game name or appid for game-specific queries"] = None) -> dict[str, Any] | None:
    ...
    with get_db() as session:
        if data_type == "list":
            ...
            friends_data = session.execute(friends_association.select().where(friends_association.c.user_steam_id == user_steam_id)).fetchall()
            ...
            friend_profiles = session.query(UserProfile).filter(UserProfile.steam_id.in_(friend_steam_ids)).all()

            # Create lookup for profiles
            profile_lookup = {profile.steam_id: profile for profile in friend_profiles}

            return {"user_steam_id": user_steam_id, "friends": [{"steam_id": friend.friend_steam_id, "persona_name": profile_lookup.get(friend.friend_steam_id).persona_name if profile_lookup.get(friend.friend_steam_id) else "Unknown", "relationship": friend.relationship, "friend_since": friend.friend_since, "profile_url": profile_lookup.get(friend.friend_steam_id).profile_url if profile_lookup.get(friend.friend_steam_id) else ""} for friend in friends_data]}
            ...
            # Find common app_ids
            common_app_ids = session.query(user_games_subq.c.app_id).intersect(session.query(friend_games_subq.c.app_id)).all()

            if not common_app_ids:
                return {"user_steam_id": user_steam_id, "friend_steam_id": friend_steam_id, "common_games": []}

            common_app_id_list = [app_id[0] for app_id in common_app_ids]

            # Get user playtime for common games
            user_playtimes = session.query(UserGame.app_id, UserGame.playtime_forever).filter(and_(UserGame.steam_id == user_steam_id, UserGame.app_id.in_(common_app_id_list))).all()

            # Get friend playtime for common games
            friend_playtimes = session.query(UserGame.app_id, UserGame.playtime_forever).filter(and_(UserGame.steam_id == friend_steam_id, UserGame.app_id.in_(common_app_id_list))).all()
            ...
            # Get game names
            games = session.query(Game.app_id, Game.name).filter(Game.app_id.in_(common_app_id_list)).all()
            ...
            for game in games:
                ...
                common_games.append({"appid": game.app_id, "name": game.name, "user_playtime_hours": round(user_playtime / 60, 1) if user_playtime else 0, "friend_playtime_hours": round(friend_playtime / 60, 1) if friend_playtime else 0})

            # Sort by user playtime (descending)
            common_games.sort(key=lambda x: x["user_playtime_hours"], reverse=True)

            return {"user_steam_id": user_steam_id, "friend_steam_id": friend_steam_id, "common_games": common_games}
            ...
            friend_steam_ids = [friend.friend_steam_id for friend in friends_data]
            if not friend_steam_ids:
                return {"user_steam_id": user_steam_id, "game": {"appid": game.app_id, "name": game.name}, "friends_with_game": []}

            # Find which friends own this game
            friends_with_game = session.query(UserProfile, UserGame).join(UserGame, UserProfile.steam_id == UserGame.steam_id).filter(and_(UserProfile.steam_id.in_(friend_steam_ids), UserGame.app_id == game.app_id)).all()

            return {"user_steam_id": user_steam_id, "game": {"appid": game.app_id, "name": game.name}, "friends_with_game": [{"steam_id": friend.UserProfile.steam_id, "persona_name": friend.UserProfile.persona_name or "Unknown", "playtime_hours": round(friend.UserGame.playtime_forever / 60, 1) if friend.UserGame.playtime_forever else 0} for friend in friends_with_game]}
