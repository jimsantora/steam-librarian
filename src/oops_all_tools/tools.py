...
async def search_games(
    ...
    if not query or not query.strip():
        return json.dumps({
            ...
            "help": "Provide a search query like 'Portal' or 'action RPG games'",
            "examples": [
                "search_games(query='Portal')",
                "search_games(query='action games rated above 80')",
                "search_games(query='multiplayer indie games')"
            ]
        }, indent=2)
    ...
    if "error" in user_result:
        return json.dumps({
            "error": f"User error: {user_result['message']}",
            "help": "Use get_user_profile() to check available users or set DEFAULT_USER"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            # Build base query
            query_obj = session.query(Game).join(UserGame).filter(
                UserGame.user_steam_id == user_steam_id
            ).options(
            ...
            if query:
                ...
                for term in search_terms:
                    query_obj = query_obj.filter(
                        or_(
                            Game.name.ilike(f"%{term}%"),
                            Game.short_description.ilike(f"%{term}%")
                        )
                    )
            ...
            if filters:
                try:
                    ...
                    if "genres" in filter_dict:
                        query_obj = query_obj.join(Game.genres).filter(
                            Genre.genre_name.in_(filter_dict["genres"])
                        )
                    ...
                    if "categories" in filter_dict:
                        query_obj = query_obj.join(Game.categories).filter(
                            Category.category_name.in_(filter_dict["categories"])
                        )

                except json.JSONDecodeError:
                    return json.dumps({
                        "error": "Invalid filters format",
                        "help": "Use JSON format like '{\"genres\": [\"Action\"], \"min_rating\": 80}' or natural language",
            ...
            for game in games:
                ...
                results.append({
                    ...
                    "rating": game.metacritic_score,
                    "playtime": user_game.playtime_forever if user_game else 0,
                    "short_description": game.short_description[:200] + "..." if game.short_description and len(game.short_description) > 200 else game.short_description
                })
        ...
        return json.dumps({
            ...
            "help": "Try a simpler query or check if the database is accessible"
        }, indent=2)
...
async def get_game_details(
    ...
    if not game_id and not game_name:
        return json.dumps({
            "error": "Missing required parameter: game_id or game_name",
            "help": "Provide either a game_id or game_name to look up",
            "discover_game_id": [
                "Use search_games(query='game name') to find the game_id",
                "Use get_user_games() to list all games with their IDs"
            ],
            "examples": [
                "get_game_details(game_id=440)",
                "get_game_details(game_name='Portal 2')",
                "First: search_games(query='Portal'), then use the game_id from results"
            ]
        }, indent=2)
    ...
    if "error" in user_result:
        return json.dumps({
            "error": f"User error: {user_result['message']}",
            "help": "Use get_user_profile() to check available users"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if not game:
                ...
                return json.dumps({
                    "error": f"Game not found: {game_name or game_id}",
                    "help": f"Try using {search_suggestion} to find the correct game",
                    "alternative": "Use get_user_games() to see all available games"
                }, indent=2)

            # Get user-specific data
            user_game = session.query(UserGame).filter(
                and_(
                    UserGame.user_steam_id == user_steam_id,
                    UserGame.appid == game.appid
                )
            ).first()
            ...
            details = {
                ...
                "price": {
                    "initial": game.price_initial,
                    "final": game.price_final,
            ...
            if user_game:
                details["user_data"] = {
                    ...
                    "last_played": user_game.last_played.isoformat() if user_game.last_played else None,
                ...
                details["reviews"] = [
                    {
                        ...
                        "helpful": r.helpful,
                        "funny": r.funny,
                        "hours_on_record": r.hours_on_record,
                        "review": r.review[:500] + "..." if len(r.review) > 500 else r.review
                    } for r in game.reviews[:5]  # Limit to 5 reviews
                ]
        ...
        return json.dumps({
            ...
            "help": "Check if the game_id is valid or try searching by name first"
        }, indent=2)
...
async def find_similar_games(
    ...
    if not game_id and not game_name:
        return json.dumps({
            "error": "Missing required parameter: game_id or game_name",
            "help": "Specify which game to find similar games to",
            "discover_game_id": [
                "Use search_games(query='game name') to find the game_id",
                "Use get_user_games() to list all your games with IDs"
            ],
            "examples": [
                "find_similar_games(game_id=440)",
                "find_similar_games(game_name='Portal 2')",
                "find_similar_games(game_id=440, similarity_factors=['genre', 'tags'])"
            ]
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if not ref_game:
                return json.dumps({
                    "error": f"Reference game not found: {game_name or game_id}",
                    "help": "Use search_games() to find the correct game first",
                    "example": f"search_games(query='{game_name or game_id}')"
                }, indent=2)

            # Get user's games for similarity matching
            user_games_query = session.query(Game).join(UserGame).filter(
                UserGame.user_steam_id == user_steam_id,
                Game.appid != ref_game.appid  # Exclude the reference game itself
            ).options(
                joinedload(Game.genres),
                joinedload(Game.categories),
                joinedload(Game.tags),
        ...
        return json.dumps({
            "error": f"Failed to find similar games: {str(e)}",
            "help": "Try with a different game or check similarity_factors"
        }, indent=2)
...
async def get_library_overview(
    ...
    if "error" in user_result:
        return json.dumps({
            "error": f"User error: {user_result['message']}",
            "help": "Use get_user_profile() to check available users or set DEFAULT_USER in config"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            # Get basic counts
            total_games = session.query(UserGame).filter(
                UserGame.user_steam_id == user_steam_id
            ).count()
            ...
            # Get total playtime
            total_playtime = session.query(func.sum(UserGame.playtime_forever)).filter(
                UserGame.user_steam_id == user_steam_id
            ).scalar() or 0
            ...
            if include_stats:
                ...
                genre_stats = session.query(
                    Genre.genre_name,
                    func.count(UserGame.appid).label('count')
                ).join(Game.genres).join(UserGame).filter(
                    UserGame.user_steam_id == user_steam_id
                ).group_by(Genre.genre_name).order_by(
                    func.count(UserGame.appid).desc()
                ).limit(10).all()

                # Top played games
                top_games = session.query(Game, UserGame).join(UserGame).filter(
                    UserGame.user_steam_id == user_steam_id,
                    UserGame.playtime_forever > 0
                ).order_by(UserGame.playtime_forever.desc()).limit(10).all()

                # Recent achievements
                recent_achievements = session.query(UserGame).filter(
                    UserGame.user_steam_id == user_steam_id,
                    UserGame.achievements_unlocked > 0
                ).order_by(UserGame.achievements_unlocked.desc()).limit(5).all()
                ...
                overview["statistics"] = {
                    ...
                    "achievement_progress": [
                        {
                            "game_id": ug.appid,
                            "achievements_unlocked": ug.achievements_unlocked,
                            "achievements_total": ug.achievements_total,
                            "completion_percent": round((ug.achievements_unlocked / ug.achievements_total * 100), 1) if ug.achievements_total > 0 else 0
                        }
                        for ug in recent_achievements
                    ]
                }
        ...
        return json.dumps({
            "error": f"Failed to get library overview: {str(e)}",
            "help": "Check if user exists and database is accessible"
        }, indent=2)
...
async def get_user_profile(
    ...
    try:
        with get_db() as session:
            if user_id:
                ...
                user = session.query(UserProfile).filter(
                    UserProfile.steam_id == user_id
                ).first()

                if not user:
                    return json.dumps({
                        "error": f"User not found: {user_id}",
                        "help": "Use get_user_profile() without parameters to see all available users",
                ...
                profile = {
                    ...
                    "lastlogoff": user.lastlogoff.isoformat() if user.lastlogoff else None,
                    "commentpermission": user.commentpermission,
                    "realname": user.realname,
                    "primaryclanid": user.primaryclanid,
                    "timecreated": user.timecreated.isoformat() if user.timecreated else None,
                ...
                if not users:
                    return json.dumps({
                        ...
                        "help": "Run the Steam fetcher to populate user data",
                ...
                user_list = [
                    {
                        ...
                        "personaname": user.personaname,
                        "profileurl": user.profileurl,
                        "is_default": user.steam_id == config.default_user
                    }
                    for user in users
                ]
        ...
        return json.dumps({
            ...
            "help": "Check database connection and user data availability"
        }, indent=2)
...
async def get_user_games(
    ...
    if "error" in user_result:
        return json.dumps({
            "error": f"User error: {user_result['message']}",
            "help": "Use get_user_profile() to see available users",
            "example": "get_user_profile()"
        }, indent=2)
    ...
    if sort_by not in valid_sorts:
        return json.dumps({
            ...
            "help": f"Use one of: {', '.join(valid_sorts)}",
            "example": "get_user_games(sort_by='playtime')"
        }, indent=2)

    try:
        with get_db() as session:
            # Build query
            query = session.query(Game, UserGame).join(UserGame).filter(
                UserGame.user_steam_id == user_steam_id
            ).options(
                joinedload(Game.genres),
                joinedload(Game.categories)
            ...
            for game, user_game in results:
                games.append({
                    ...
                    "name": game.name,
                    "genres": [g.genre_name for g in game.genres],
                    "categories": [c.category_name for c in game.categories[:5]],  # Limit categories
                    "playtime_forever": user_game.playtime_forever,
                    "playtime_hours": round(user_game.playtime_forever / 60, 1),
                    "playtime_2weeks": user_game.playtime_2weeks,
                    "last_played": user_game.last_played.isoformat() if user_game.last_played else None,
                    "metacritic_score": game.metacritic_score,
                    "achievements": {
                        "unlocked": user_game.achievements_unlocked,
                        "total": user_game.achievements_total,
                        "percentage": round((user_game.achievements_unlocked / user_game.achievements_total * 100), 1) if user_game.achievements_total > 0 else 0
                    }
                })
            ...
            return json.dumps({
                ...
                "showing": f"{len(games)} of total games"
            }, indent=2)

    except Exception as e:
        return json.dumps({
            "error": f"Failed to get user games: {str(e)}",
            "help": "Check if user exists and has games in the database"
        }, indent=2)
...
async def get_user_stats(
    ...
    if time_range not in valid_ranges:
        return json.dumps({
            "error": f"Invalid time_range: {time_range}",
            "help": f"Use one of: {', '.join(valid_ranges)}",
    ...
    try:
        with get_db() as session:
            ...
            base_query = session.query(UserGame).filter(
                UserGame.user_steam_id == user_steam_id
            )
            ...
            # Playtime statistics
            total_playtime = base_query.with_entities(
                func.sum(UserGame.playtime_forever)
            ).scalar() or 0

            if time_range == "recent":
                recent_playtime = base_query.with_entities(
            ...
            achievement_stats = base_query.with_entities(
                func.sum(UserGame.achievements_unlocked),
                func.sum(UserGame.achievements_total),
                func.count(UserGame.appid)
            ).first()

            achievements_unlocked = achievement_stats[0] or 0
            achievements_total = achievement_stats[1] or 0
            games_with_achievements = base_query.filter(
            ...
            genre_playtime = session.query(
                ...
                func.sum(UserGame.playtime_forever).desc()
            ).limit(10).all()
            ...
            stats = {
                ...
                "library_summary": {
                    ...
                    "played_games": played_games,
                    "unplayed_games": total_games - played_games,
                    "completion_rate": round((played_games / total_games * 100), 1) if total_games > 0 else 0
                },
                "playtime_stats": {
                    "total_minutes": total_playtime,
                    "total_hours": round(total_playtime / 60, 1),
                    "total_days": round(total_playtime / (60 * 24), 1),
                    "average_per_game": round(total_playtime / total_games, 1) if total_games > 0 else 0,
                    "average_per_played_game": round(total_playtime / played_games, 1) if played_games > 0 else 0
                },
                "achievement_stats": {
                    "total_unlocked": achievements_unlocked,
                    "total_available": achievements_total,
                    "completion_rate": round((achievements_unlocked / achievements_total * 100), 1) if achievements_total > 0 else 0,
                    "games_with_achievements": games_with_achievements,
                    "average_per_game": round(achievements_unlocked / games_with_achievements, 1) if games_with_achievements > 0 else 0
                },
                "genre_preferences": [
                    {
                        "genre": genre,
                        "playtime_minutes": int(playtime),
                        "playtime_hours": round(playtime / 60, 1),
                        "percentage_of_total": round((playtime / total_playtime * 100), 1) if total_playtime > 0 else 0
                    }
                    for genre, playtime in genre_playtime
                ]
            }
            ...
            if time_range == "recent" and recent_playtime > 0:
                stats["recent_activity"] = {
                    "playtime_2weeks_minutes": recent_playtime,
                    "playtime_2weeks_hours": round(recent_playtime / 60, 1),
                    "daily_average": round(recent_playtime / 14, 1)
                }
    ...
    try:
        with get_db() as session:
            if include_counts:
                ...
                genre_data = session.query(
                    Genre.genre_name,
                    func.count(Game.appid).label('game_count')
                ).join(Game.genres).group_by(
                    Genre.genre_name
                ).order_by(
                    func.count(Game.appid).desc()
                ).all()
                ...
                genres = [
                    ...
                    for genre_name, game_count in genre_data
                ]
            else:
                # Just genre names
...
async def get_games_by_genre(
    ...
    if not genre_name:
        ...
        return json.dumps({
            "error": "Missing required parameter: genre_name",
            "help": "Specify which genre to search for",
            "discover_genres": "Use get_genres() to see all available genres",
            "common_genres": available_genres,
            "examples": [
                "get_games_by_genre(genre_name='Action')",
                "get_games_by_genre(genre_name='RPG', sort_by='rating')"
            ]
        }, indent=2)
    ...
    if sort_by not in valid_sorts:
        return json.dumps({
            "error": f"Invalid sort_by: {sort_by}",
            "help": f"Use one of: {', '.join(valid_sorts)}",
            "example": "get_games_by_genre(genre_name='Action', sort_by='rating')"
        }, indent=2)

    try:
        with get_db() as session:
            # Verify genre exists
            genre_exists = session.query(Genre).filter(
                Genre.genre_name.ilike(genre_name)
            ).first()

            if not genre_exists:
                # Suggest similar genres
                similar_genres = session.query(Genre.genre_name).filter(
                    Genre.genre_name.ilike(f"%{genre_name}%")
                ).limit(5).all()

                return json.dumps({
                    "error": f"Genre not found: {genre_name}",
                    "help": "Use get_genres() to see all valid genres",
                    "suggestions": [g[0] for g in similar_genres] if similar_genres else [],
                    "example": "get_genres()"
                }, indent=2)

            # Build query
            query = session.query(Game, UserGame).join(UserGame).join(Game.genres).filter(
                UserGame.user_steam_id == user_steam_id,
                Genre.genre_name.ilike(genre_name)
            ).options(
                joinedload(Game.genres),
            ...
            for game, user_game in results:
                games.append({
                    ...
                    "name": game.name,
                    "genres": [g.genre_name for g in game.genres],
                    "categories": [c.category_name for c in game.categories[:3]],
                    "playtime_hours": round(user_game.playtime_forever / 60, 1),
                    "metacritic_score": game.metacritic_score,
                    "short_description": game.short_description[:150] + "..." if game.short_description and len(game.short_description) > 150 else game.short_description
                })
        ...
        return json.dumps({
            "error": f"Failed to get games by genre: {str(e)}",
            "help": "Check if genre name is correct and user has games in this genre"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if category_type == "multiplayer":
                ...
                query = query.filter(
                    or_(*[Category.category_name.ilike(f"%{term}%") for term in multiplayer_terms])
                )
            elif category_type == "platform":
                platform_terms = ["Windows", "Mac", "Linux", "VR", "Steam"]
                query = query.filter(
                    or_(*[Category.category_name.ilike(f"%{term}%") for term in platform_terms])
                )
            ...
            if include_counts:
                ...
                category_data = query.add_columns(
                    func.count(Game.appid).label('game_count')
                ).join(Game.categories).group_by(
                ...
                categories = [
                    ...
                    for category_name, game_count in category_data
                ]
            else:
                # Just category names
            ...
            return json.dumps({
                ...
                "include_counts": include_counts,
                "filter_options": ["all", "multiplayer", "platform", "features"]
            }, indent=2)
...
async def get_games_by_category(
    ...
    if not category:
        return json.dumps({
            ...
            "help": "Specify which category to search for",
            "discover_categories": [
                "Use get_categories() to see all available categories",
                "Use get_categories(category_type='multiplayer') for multiplayer options",
                "Use get_categories(category_type='features') for game features"
            ],
            "common_categories": ["Co-op", "Multi-player", "Single-player", "VR Support", "Controller Support"],
            "examples": [
                "get_games_by_category(category='Co-op')",
                "get_games_by_category(category='VR Support')"
            ]
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if not category_exists:
                ...
                similar_categories = session.query(Category.category_name).filter(
                    Category.category_name.ilike(f"%{category}%")
                ).limit(5).all()

                return json.dumps({
                    "error": f"Category not found: {category}",
                    "help": "Use get_categories() to see all valid categories",
                    "suggestions": [c[0] for c in similar_categories] if similar_categories else [],
                    "example": "get_categories()"
                }, indent=2)

            # Build query
            results = session.query(Game, UserGame).join(UserGame).join(Game.categories).filter(
                UserGame.user_steam_id == user_steam_id,
                Category.category_name.ilike(f"%{category}%")
            ).options(
                joinedload(Game.genres),
                joinedload(Game.categories)
            ).order_by(Game.name).limit(limit).all()
            ...
            for game, user_game in results:
                games.append({
                    ...
                    "name": game.name,
                    "genres": [g.genre_name for g in game.genres],
                    "categories": [c.category_name for c in game.categories if category.lower() in c.category_name.lower()][:3],
                    "playtime_hours": round(user_game.playtime_forever / 60, 1),
                    "metacritic_score": game.metacritic_score,
                    "short_description": game.short_description[:150] + "..." if game.short_description and len(game.short_description) > 150 else game.short_description
                })
        ...
        return json.dumps({
            "error": f"Failed to get games by category: {str(e)}",
            "help": "Check if category name is correct and user has games with this category"
        }, indent=2)
...
async def recommend_games(
    ...
    try:
        with get_db() as session:
            ...
            base_query = session.query(Game, UserGame).join(UserGame).filter(
                UserGame.user_steam_id == user_steam_id
            ).options(
                joinedload(Game.genres),
                joinedload(Game.categories)
            ...
            if context == "after_work":
                ...
                base_query = base_query.join(Game.categories).filter(
                    or_(
                        Category.category_name.ilike("%Casual%"),
                        Category.category_name.ilike("%Relaxing%")
                    )
                ).filter(UserGame.playtime_forever.between(0, 300))  # Less than 5 hours total
            elif context == "social":
                # Multiplayer games
                base_query = base_query.join(Game.categories).filter(
                    or_(
                        Category.category_name.ilike("%Co-op%"),
                        Category.category_name.ilike("%Multi-player%")
                    )
                )
            elif context == "family":
                # Family-friendly games
                base_query = base_query.join(Game.genres).filter(
                    Genre.genre_name.in_(["Casual", "Puzzle", "Simulation"])
                )
            elif context == "solo":
                # Single-player focused
                base_query = base_query.join(Game.categories).filter(
                    Category.category_name.ilike("%Single-player%")
                )

            # Apply preference filters
            if "genres" in pref_filters:
                base_query = base_query.join(Game.genres).filter(
                    Genre.genre_name.in_(pref_filters["genres"])
                )
            if "min_rating" in pref_filters:
                base_query = base_query.filter(
                    Game.metacritic_score >= pref_filters["min_rating"]
                )

            # Prioritize unplayed or lightly played games
            recommendations = base_query.filter(
                UserGame.playtime_forever < 60  # Less than 1 hour played
            ).order_by(
                Game.metacritic_score.desc().nullslast(),
                UserGame.playtime_forever.asc()
            ).limit(limit).all()

            # If not enough unplayed games, include some played games
            if len(recommendations) < limit:
                additional = base_query.filter(
        ...
        return json.dumps({
            ...
            "help": "Try with different context or check if user has games in library"
        }, indent=2)
...
async def find_family_games(
    ...
    if child_age is None:
        return json.dumps({
            "error": "Missing required parameter: child_age",
            "help": "Specify child's age (3-18) for appropriate content filtering",
            "safety_note": "Age is required to ensure appropriate content recommendations",
            "examples": [
                "find_family_games(child_age=8)",
                "find_family_games(child_age=12, content_preferences=['educational', 'no_violence'])"
            ]
        }, indent=2)

    if not (3 <= child_age <= 18):
        return json.dumps({
            "error": f"Invalid child_age: {child_age}",
            "help": "Child age must be between 3 and 18 years",
            "example": "find_family_games(child_age=10)"
        }, indent=2)
        ...
        with get_db() as session:
            ...
            if child_age <= 8:
                ...
                query = query.join(Game.genres).filter(
                    Genre.genre_name.in_(safe_genres)
                )
            elif child_age <= 12:
                ...
                query = query.join(Game.genres).filter(
                    ~Genre.genre_name.in_(avoid_genres)
                )
            ...
            if "educational" in content_preferences:
                query = query.filter(
                    or_(
                        Game.name.ilike("%education%"),
                        Game.name.ilike("%learning%"),
                        Game.short_description.ilike("%educational%")
                    )
                )

            # Family-friendly categories
            family_categories = ["Family Sharing", "Single-player", "Local Co-op"]
            query = query.join(Game.categories).filter(
                Category.category_name.in_(family_categories)
            )
            ...
            for game, user_game in results:
                ...
                games.append({
                    ...
                    "name": game.name,
                    "genres": [g.genre_name for g in game.genres],
                    "age_appropriateness": "Suitable" if len(safety_factors) >= 2 else "Review recommended",
            ...
            return json.dumps({
                ...
                "safety_note": "Games filtered for age-appropriate content",
                "recommendation": "Always review games before allowing children to play",
                "count": len(games)
            }, indent=2)

    except Exception as e:
        return json.dumps({
            "error": f"Failed to find family games: {str(e)}",
            "help": "Check if user has family-appropriate games in library"
        }, indent=2)


@mcp.tool()
async def find_quick_games(
    ...
    if session_length not in valid_lengths:
        return json.dumps({
            "error": f"Invalid session_length: {session_length}",
            "help": f"Use one of: {', '.join(valid_lengths)}",
            "definitions": {
                "short": "15-30 minutes",
                "medium": "30-60 minutes",
                "long": "60+ minutes"
            },
            "example": "find_quick_games(session_length='short', genre_preference='Puzzle')"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if genre_preference:
                # Verify genre exists
                genre_exists = session.query(Genre).filter(
                    Genre.genre_name.ilike(genre_preference)
                ).first()

                if not genre_exists:
                    return json.dumps({
                        "error": f"Genre not found: {genre_preference}",
                        "help": "Use get_genres() to see all available genres",
                        "example": "get_genres()"
                    }, indent=2)
            ...
            results = query.order_by(
                ...
                case(
                    (UserGame.playtime_forever.between(60, 600), 1),  # 1-10 hours
                    else_=2
                ).asc(),
                Game.metacritic_score.desc().nullslast(),
                Game.name
            ...
            for game, user_game in results:
                ...
                games.append({
                    ...
                    "name": game.name,
                    "genres": game_genres,
                    "session_suitability": suitability_factors,
                    "playtime_hours": round(user_game.playtime_forever / 60, 1),
                    "metacritic_score": game.metacritic_score,
                    ...
                    "short_description": game.short_description[:120] + "..." if game.short_description and len(game.short_description) > 120 else game.short_description
                })
            ...
            return json.dumps({
                ...
                "count": len(games),
                "session_tips": {
                    "short": "Perfect for breaks, commutes, or quick entertainment",
                    "medium": "Good for evening relaxation or weekend gaming",
                    "long": "Great for deep dives and immersive experiences"
                }[session_length]
            }, indent=2)

    except Exception as e:
        return json.dumps({
            "error": f"Failed to find quick games: {str(e)}",
            "help": "Check session_length parameter and user's library"
        }, indent=2)


@mcp.tool()
async def get_unplayed_games(
    user: str | None = None,
    sort_by: str = "rating",
    ...
    if sort_by not in valid_sorts:
        return json.dumps({
            "error": f"Invalid sort_by: {sort_by}",
            "help": f"Use one of: {', '.join(valid_sorts)}",
            "example": "get_unplayed_games(sort_by='rating')"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if not results:
                return json.dumps({
                    "message": "No unplayed games found!",
                    "achievement": "You've tried all your games - impressive!",
                    "suggestion": "Consider getting some new games or revisiting old favorites",
                    "alternative": "Use get_user_games(filter_played=True, sort_by='playtime') to find lightly played games"
                }, indent=2)
            ...
            return json.dumps({
                ...
                "user_steam_id": user_steam_id,
                "sort_by": sort_by,
                "backlog_size": len(games),
                "encouragement": "Your gaming backlog is a treasure trove of experiences!",
                "tip": "Try starting with highly-rated games in your favorite genres"
            }, indent=2)

    except Exception as e:
        return json.dumps({
            "error": f"Failed to get unplayed games: {str(e)}",
            "help": "Check if user has games in library"
        }, indent=2)
...
async def get_platform_games(
    ...
    if not platform:
        return json.dumps({
            ...
            "help": "Specify which platform to search for",
            "available_platforms": ["windows", "mac", "linux", "steam_deck", "vr"],
            "examples": [
                "get_platform_games(platform='mac')",
                "get_platform_games(platform='steam_deck')"
            ]
        }, indent=2)
    ...
    if platform_lower not in valid_platforms:
        return json.dumps({
            ...
            "help": f"Use one of: {', '.join(valid_platforms)}",
            "example": "get_platform_games(platform='windows')"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if platform_lower == "windows":
                ...
                query = query.join(Game.categories).filter(
                    or_(
                        Category.category_name.ilike("%Steam Deck%"),
                        Category.category_name.ilike("%Deck Verified%"),
                        Category.category_name.ilike("%Great on Deck%")
                    )
                )
            elif platform_lower == "vr":
                # VR games
                query = query.join(Game.categories).filter(
                    Category.category_name.ilike("%VR%")
                )
            ...
            return json.dumps({
                ...
                "count": len(games),
                "user_steam_id": user_steam_id,
                "compatibility_note": f"Games verified or compatible with {platform_display}"
            }, indent=2)

    except Exception as e:
        return json.dumps({
            "error": f"Failed to get platform games: {str(e)}",
            "help": "Check if platform name is correct"
        }, indent=2)


@mcp.tool()
async def get_multiplayer_games(
    multiplayer_type: str | None = None,
    user: str | None = None
) -> str:
    """
    ...
    if not multiplayer_type:
        return json.dumps({
            "error": "Missing required parameter: multiplayer_type",
            "help": "Specify type of multiplayer games to find",
            "discover_multiplayer_types": "Use get_categories(category_type='multiplayer') to see all multiplayer options",
            "common_types": ["co-op", "pvp", "online", "local", "lan"],
            "examples": [
                "get_multiplayer_games(multiplayer_type='co-op')",
                "get_multiplayer_games(multiplayer_type='local')"
            ]
        }, indent=2)
    ...
    type_mapping = {
        "co-op": ["Co-op", "Online Co-op", "Local Co-op"],
        "coop": ["Co-op", "Online Co-op", "Local Co-op"],
        "pvp": ["PvP"],
        "online": ["Online", "Online Multi-Player", "Online Co-op", "Online PvP"],
        "local": ["Local Co-op", "Local Multi-Player", "Shared/Split Screen"],
        "lan": ["LAN"],
        "multiplayer": ["Multi-player", "Online Multi-Player"]
    }

    if mp_type not in type_mapping:
        return json.dumps({
            "error": f"Invalid multiplayer_type: {multiplayer_type}",
            "help": f"Use one of: {', '.join(type_mapping.keys())}",
            "discover": "Use get_categories(category_type='multiplayer') for all options",
            "example": "get_multiplayer_games(multiplayer_type='co-op')"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            # Build query
            query = session.query(Game, UserGame).join(UserGame).join(Game.categories).filter(
                UserGame.user_steam_id == user_steam_id,
                or_(*[Category.category_name.ilike(f"%{pattern}%") for pattern in category_patterns])
            ).options(
                joinedload(Game.genres),
                joinedload(Game.categories)
            ).distinct()
            ...
            for game, user_game in results:
                ...
                games.append({
                    ...
                    "name": game.name,
                    "genres": [g.genre_name for g in game.genres],
                    "multiplayer_features": multiplayer_features,
                    "playtime_hours": round(user_game.playtime_forever / 60, 1),
                    "metacritic_score": game.metacritic_score,
                    "social_recommendation": {
                        "co-op": "Great for playing with friends cooperatively",
                        "pvp": "Competitive multiplayer experience",
                        "online": "Connect with players worldwide",
                        "local": "Perfect for couch gaming with friends",
                        "lan": "Ideal for LAN parties"
                    }.get(mp_type, "Multiplayer gaming experience"),
                    "short_description": game.short_description[:150] + "..." if game.short_description and len(game.short_description) > 150 else game.short_description
                })
            ...
            return json.dumps({
                ...
                "social_gaming_tip": {
                    "co-op": "Cooperative games are great for building teamwork",
                    "pvp": "Competitive games can be intense - remember to have fun!",
                    "local": "Local multiplayer is perfect for parties and gatherings"
                }.get(mp_type, "Multiplayer games are more fun with friends!")
            }, indent=2)

    except Exception as e:
        return json.dumps({
            "error": f"Failed to get multiplayer games: {str(e)}",
            "help": "Check if multiplayer_type is correct"
        }, indent=2)


@mcp.tool()
async def get_vr_games(
    vr_type: str = "any",
    user: str | None = None
) -> str:
    """
    ...
    if vr_type not in valid_vr_types:
        return json.dumps({
            "error": f"Invalid vr_type: {vr_type}",
            "help": f"Use one of: {', '.join(valid_vr_types)}",
            "definitions": {
                "any": "All VR games",
                "seated": "Games playable while sitting",
                "room_scale": "Games requiring room-scale movement",
                "motion_controllers": "Games requiring hand controllers"
            },
            "example": "get_vr_games(vr_type='seated')"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if vr_type == "seated":
                ...
                query = query.filter(
                    or_(
                        Category.category_name.ilike("%Seated%"),
                        Category.category_name.ilike("%VR Supported%")  # Usually seated-friendly
                    )
                )
            elif vr_type == "room_scale":
                # Look for room-scale indicators
                query = query.filter(
                    Category.category_name.ilike("%Room%")
                )
            elif vr_type == "motion_controllers":
                # Look for motion controller requirements
                query = query.filter(
                    or_(
                        Category.category_name.ilike("%Motion%"),
                        Category.category_name.ilike("%Controllers%")
                    )
                )
            ...
            if not results:
                return json.dumps({
                    "message": "No VR games found in your library",
                    "suggestion": "Consider purchasing VR games from the Steam VR section",
                    "alternative": "Use get_games_by_category(category='VR Support') to see all VR-related games",
                    "vr_type": vr_type
                }, indent=2)
            ...
            for game, user_game in results:
                ...
                games.append({
                    ...
                    "metacritic_score": game.metacritic_score,
                    "vr_comfort_level": {
                        "seated": "High - comfortable for all users",
                        "room_scale": "Medium - requires space and movement",
                        "motion_controllers": "Medium - requires hand coordination",
                        "any": "Varies by game"
                    }[vr_type],
                    "short_description": game.short_description[:120] + "..." if game.short_description and len(game.short_description) > 120 else game.short_description
                })
            ...
            return json.dumps({
                ...
                "user_steam_id": user_steam_id,
                "vr_tips": {
                    "any": "Make sure your VR headset is properly set up",
                    "seated": "Perfect for longer gaming sessions without fatigue",
                    "room_scale": "Ensure you have adequate play space cleared",
                    "motion_controllers": "Check controller battery levels before playing"
                }[vr_type],
                "safety_reminder": "Take breaks every 30-60 minutes when playing VR games"
            }, indent=2)

    except Exception as e:
        return json.dumps({
            "error": f"Failed to get VR games: {str(e)}",
            "help": "Check if user has VR games in library"
        }, indent=2)
...
async def analyze_gaming_patterns(
    ...
    if analysis_type not in valid_analyses:
        return json.dumps({
            ...
            "help": f"Use one of: {', '.join(valid_analyses)}",
            "descriptions": {
                "overview": "General gaming pattern overview",
                "genre_trends": "Genre preference analysis",
                "achievements": "Achievement hunting patterns",
                "playtime": "Playtime distribution analysis"
            },
            "example": "analyze_gaming_patterns(analysis_type='genre_trends')"
        }, indent=2)
    ...
    if time_range not in valid_ranges:
        return json.dumps({
            "error": f"Invalid time_range: {time_range}",
            "help": f"Use one of: {', '.join(valid_ranges)}",
            "example": "analyze_gaming_patterns(analysis_type='overview', time_range='recent')"
        }, indent=2)
    ...
    try:
        with get_db() as session:
            ...
            if analysis_type == "overview":
                ...
                analysis["overview"] = {
                    ...
                    "total_hours": round(total_playtime / 60, 1),
                    "average_per_game": round(total_playtime / total_games, 1) if total_games > 0 else 0,
                    "gaming_style": "Completionist" if played_games / total_games > 0.7 else "Collector" if total_games > 100 else "Focused Gamer"
                }
                ...
                genre_data = session.query(
                    Genre.genre_name,
                    func.count(UserGame.appid).label('game_count'),
                    func.sum(UserGame.playtime_forever).label('total_playtime'),
                ...
                analysis["genre_trends"] = [
                    {
                        "genre": genre,
                        "games_owned": int(count),
                        "total_hours": round(playtime / 60, 1),
                        "average_hours_per_game": round(avg_playtime / 60, 1),
                        "preference_score": round((playtime / 60) / count, 1) if count > 0 else 0
                    }
                    for genre, count, playtime, avg_playtime in genre_data[:10]
                ]

            elif analysis_type == "achievements":
                # Achievement pattern analysis
                achievement_data = session.query(
                    func.sum(UserGame.achievements_unlocked).label('total_unlocked'),
                    func.sum(UserGame.achievements_total).label('total_available'),
                    func.count(UserGame.appid).label('games_with_achievements')
                ).filter(
                    UserGame.user_steam_id == user_steam_id,
                    UserGame.achievements_total > 0
                ).first()
                ...
                top_achievement_games = session.query(Game.name, UserGame.achievements_unlocked, UserGame.achievements_total).join(UserGame).filter(
                    UserGame.user_steam_id == user_steam_id,
                    UserGame.achievements_total > 0
                ).order_by(UserGame.achievements_unlocked.desc()).limit(10).all()

                analysis["achievement_patterns"] = {
                    "total_unlocked": achievement_data.total_unlocked or 0,
                    "total_available": achievement_data.total_available or 0,
                    "completion_rate": round((achievement_data.total_unlocked or 0) / (achievement_data.total_available or 1) * 100, 1),
                    "games_with_achievements": achievement_data.games_with_achievements or 0,
                    "hunter_level": "Achievement Hunter" if (achievement_data.total_unlocked or 0) > 500 else "Casual" if (achievement_data.total_unlocked or 0) > 100 else "Beginner",
                    "top_games": [
                        {
                            "game": name,
                            "unlocked": unlocked,
                        ...
                        for name, unlocked, total in top_achievement_games
                    ]
                }
                ...
                playtime_brackets = [
                    ...
                    ("Tried (< 1hr)", 1, 59),
                    ("Short (1-5hrs)", 60, 299),
                    ("Medium (5-20hrs)", 300, 1199),
                    ("Long (20-50hrs)", 1200, 2999),
                    ("Epic (50+ hrs)", 3000, float('inf'))
                ]

                playtime_distribution = []
                for bracket_name, min_time, max_time in playtime_brackets:
                    if max_time == float('inf'):
                        count = session.query(UserGame).filter(
                            UserGame.user_steam_id == user_steam_id,
                            UserGame.playtime_forever >= min_time
                        ).count()
                    else:
                        count = session.query(UserGame).filter(
                            UserGame.user_steam_id == user_steam_id,
                            UserGame.playtime_forever.between(min_time, max_time)
                        ).count()

                    playtime_distribution.append({
                        "bracket": bracket_name,
                        "count": count,
                        "range": f"{min_time//60}h - {max_time//60 if max_time != float('inf') else '∞'}h"
                    })
        ...
        return json.dumps({
            "error": f"Failed to analyze gaming patterns: {str(e)}",
            "help": "Check analysis parameters and user data availability"
        }, indent=2)
