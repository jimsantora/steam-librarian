...
async def info(request: Request) -> JSONResponse:
    ...
    return JSONResponse({
        "name": SERVER_NAME,
        "version": "1.0.0",
        "description": "Tools-only MCP server for Steam library management",
        "type": "tools-only",
        "port": config.port,
        "features": {
            "tools": True,
            "resources": False,
            "prompts": True,  # Simple text examples
            "completions": False,
            "elicitations": False,
            "sampling": False
        },
        ...
        "companion_server": {
            "full_server_port": 8000,
            "features": "resources, completions, elicitations, sampling"
        }
    })
